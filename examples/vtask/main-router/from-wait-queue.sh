...
# vtask 资源分配 + 转发
# 使用 vtask bind 绑定计算资源组，替代原 semagroup max + decrement 两步
...
if [ "$TASK_DIST_MODE" = "HOST-BOUND" ]; then
    ...
    scalebox vtask add-subtask --app-id=$APP_ID --module=vtask-head \
    ...
    to_slot_index=$(scalebox vtask bind --app-id=$APP_ID) || exit 1
