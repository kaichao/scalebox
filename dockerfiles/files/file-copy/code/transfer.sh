...
# ===== LOCAL -> SSH =====
transfer_LOCAL_SSH() {
    local ssh_cmd=$(get_ssh_cmd "$2" "target_url" "target_jump")
    echo "[DEBUG] ssh_cmd:$ssh_cmd" >> auxout.txt
    local local_file=$(get_host_path "$source_dir/$1")

    local remote_file="$target_dir/$1"

    # source file not exists ?
    [ ! -f "$local_file" ] && echo "file $local_file not exists, exit " >&2 && exit 101

    ensure_remote_dir "$ssh_cmd" "$(dirname $remote_file)"

    local cmd="cat ${local_file} | pv -q | $ssh_cmd \"cat > $remote_file\""
...
# ===== LOCAL -> RSYNC_OVER_SSH =====
transfer_LOCAL_RSYNC_OVER_SSH() {
    echo "LOCAL to RSYNC_OVER_SSH"
    local target_ssh_option=$(get_ssh_option "$2" "target_url" "target_jump")
    ...
    local target_ssh_url=$(to_ssh_url $target_url)
    local target_dir_rsync=$(dirname ${target_ssh_url#*:}/$1)
    echo target_dir:$target_dir_rsync
...
transfer_LOCAL_RSYNC() {
    ...
    local rsync_url=$(to_rsync_url "$target_url")

    if [ "$keep_source" = "no" ]; then
        rsync_args=" --remove-source-files ${rsync_args} "
    fi
    local local_dir=$(get_host_path "${source_dir}")
...
transfer_SSH_LOCAL() {
    echo "SSH to LOCAL"
    local ssh_cmd=$(get_ssh_cmd "$2" "source_url" "source_jump")
    echo "[DEBUG] ssh_cmd:$ssh_cmd" >> ${WORK_DIR}/auxout.txt
    local local_file=$(get_host_path "$target_dir/$1")
    local remote_file="$source_dir/$1"

    ensure_local_dir "$(dirname $local_file)"

    local cmd="$ssh_cmd \"cat < $remote_file\" - | pv -q > ${local_file}"
    echo "[DEBUG] cmd:$cmd" >> ${WORK_DIR}/auxout.txt
    eval "$cmd"; local code=$?
...
transfer_SSH_SSH() {
    echo "SSH to SSH"
    local source_ssh_cmd=$(get_ssh_cmd "$2" "source_url" "source_jump")
    local target_ssh_cmd=$(get_ssh_cmd "$2" "target_url" "target_jump")
...
transfer_RSYNC_OVER_SSH_LOCAL() {
    echo "RSYNC_OVER_SSH to LOCAL"
    local source_ssh_option=$(get_ssh_option "$2" "source_url" "source_jump")
    local local_file=$(get_host_path "$target_dir/$1")
    ...
    mkdir -p ${dest_dir}
    cd ${dest_dir}
    local source_ssh_url=$(to_ssh_url $source_url)
    ...
    if [ "$keep_source" = "no" ]; then
        local cmd_rm="ssh ${source_ssh_option} $(get_ssh_host $source_url) rm -f $remote_file"
...
transfer_RSYNC_LOCAL() {
    ...
    mkdir -p ${dest_dir}
    local rsync_url=$(to_rsync_url "$source_url")
    export RSYNC_PASSWORD=cas12345
    if [ "$keep_source" = "no" ]; then
        rsync_args=" --remove-source-files ${rsync_args} "
    fi
