...
if [[ $ACTION == 'PUSH' ]]; then
    ...
    if [[ $ENABLE_RECHECK_SIZE == 'yes' ]]; then
        ...
        [[ $local_size -ne $remote_size ]] && echo inconsistent file size, local_size:$local_size, remote_size:$remote_size, exit_code:13 >&2 && exit 13
    fi
    ...
    cmd="cp $local_dir/$filename /work"
    eval $cmd; code=$?
    [[ $code -ne 0 ]] && echo cmd:$cmd, ret_code:$code, exit_code:31 >&2 && exit 31
    ...
    eval $cmd_put_file; code=$?
    if [[ $code -ne 0 ]]; then
        echo cmd:$cmd, ret_code:$code, exit_code:32 >&2
    ...
    eval $cmd_get_file; code=$?
    if [[ $code -ne 0 ]]; then
        echo cmd:$cmd, ret_code:$code, exit_code:33 >&2
        rm -f /work/${filename} /work/${filename}.orig
        exit 33
    fi
