...
| vtask_size   | standard flow control parameter, defines the upper limit of the number of vtasks that can be simultaneously in ready/running states; when the app is parsed, the corresponding semaphore and initial value are created.<br/>The semaphore name for global vtask flow control is: ```vtask_size:${mod_name}```;<br/>The semaphore name for SLOT-BOUND vtask flow control is: ```slot_vtask_size:${mod_name}:${slot_id}:${slot_seq}```;<br/>The semaphore name for HOST-BOUND vtask flow control is: ```host_vtask_size:${mod_name}:${hostname}``` |
