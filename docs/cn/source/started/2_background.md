...
- 传统计算框架的局限性（大数据处理、HPC）

- 云原生与容器化技术的发展

- 大规模科学计算面临的挑战（数据规模、I/O瓶颈、算法复杂）
...
- 以本地存储优化整体I/O路径
- 不增加已有硬件成本，大大提升I/O效率
- 用本机内存缓存、本地磁盘、共享磁盘，组成分级存储体系；本地存储支持I/O线性扩展。
- 以流水线并行实现不同等级存储间迁移，降低对算法运行的影响

### 两级编程模型

- 并行逻辑与算法逻辑分离，以容器化技术作为主要技术手段
- 算法逻辑主要关注单节点算法实现，不用刻意关注多节点并行
- 并行逻辑通过单实例串行实现，从全局协调并行计算
- 三类并行优化方法：
  - 算法模块内代码并行
  - 模块级数据并行
  - 模块间流水线并行
...
- 容错机制主要针对硬件故障、非逻辑错的处理。
- 要求影响范围小、恢复速度快。

#### 错误分类
- 硬件故障：不可避免
  - 节点从当前运行中退出，导致需手工调整节点。
  - 
- 软件逻辑错：
  - slot暂时故障，task连续出错。需一段时间恢复后，slot还可以正常运行task
  - 重启主机后恢复（？）
  - 
- 非逻辑错
  - 软件运行中task出错，可通过重试恢复。按一定概率发生。
...
#### 人工介入处理
- 设置任务级容错规则
- 删除出错的任务执行纪录，触发任务状态的自动修改 err_code -> 'READY'
- 修改出错的slot状态 'ERROR' -> 'READY'
